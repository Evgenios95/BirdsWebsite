...
function incrementGames() {
  retrieveSessions().then((arrayOfSessions) => {
...
async function retrieveAttempts() {
  let arrayOfSessions = await retrieveSessions();
...
async function getLostSessions() {
  let arrayOfSessions = await retrieveSessions();
...
async function renderData() {
  ...
  if (nameHere === undefined) {
    ...
    attemptsHere.innerHTML = "Number of attempts:" + results[0];
    winsHere.innerHTML = "Number of wins:" + results[1];
    lossesHere.innerHTML = "Number of losses:" + results[2];
  }
